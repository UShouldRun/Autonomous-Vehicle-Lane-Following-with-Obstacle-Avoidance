...
def make_objective(agent: str, env: WebotsLaneEnv):
    ...
    def objective(trial: optuna.Trial) -> float:
        # 1. Reset the environment state (teleports car, clears sensors).
        #    Also wipe the episode history so this trial's score is isolated.
        ...
        # 3. Build a brand-new model (cheap) bound to the existing env.
        model = AgentCls(
        ...
        # 5. Score: mean reward of the last REWARD_WINDOW completed episodes.
...
def tune(agent: str, base_cfg: dict) -> None:
    ...
    # ── Build the single shared environment for this agent ─────────
