...
class LaneCNN(BaseFeaturesExtractor):
    ...
    Replaces SB3's default NatureCNN, which uses large strides (8,4,2) designed
    for 84x84 Atari frames. Those strides are too aggressive for road scenes:
    they discard the fine horizontal detail needed to localise a narrow yellow
    line. This extractor uses smaller strides and more filters to preserve
    ...
    Compared to NatureCNN:
        NatureCNN:  3 layers, strides [4,2,1], features_dim=512
        LaneCNN:    5 layers, strides [1,2,2,2,2], features_dim=512
        The first stride-1 layer lets the network see raw colour gradients
        (yellow vs grey) before any spatial information is lost.

    The lidar and state vectors are processed by CombinedExtractor's default
    MLP branches — we only override the camera head.
    """
...
def main():
    ...
    if AgentCls is PPO:
        ...
        policy_kwargs = dict(
            ...
            # Deeper policy/value heads to match the richer feature space
